...
class ReActCompletionModel(JsonCompletionModel):
    ...
    max_actions: Optional[int] = Field(
        default=None, description="The maximum number of actions to include in the response"
    )
...
""")
    ...
    async def _validate_completion(
        ...
        try:
            ...
            if self.max_actions and len(action_blocks) > self.max_actions:
                logger.warning(
                    f"The response contains {len(action_blocks)} actions, but the max_actions is set to {self.max_actions}. The response will be truncated."
                )
